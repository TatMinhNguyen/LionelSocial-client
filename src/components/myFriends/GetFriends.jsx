  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        await handleGetFriends(); // Đợi API handleGetFriends hoàn tất
        await handleGetSuggestions(); // Gọi API handleGetSuggestions sau đó
      } catch (error) {
        console.error(error);
      }
    };
  ...
  return (
    <div className='py-2 px-10'>
      {friends?.length <= 0 && suggestions?.length <= 0 ? (
        <div className='grid grid-cols-4 gap-4 mt-[28px]'>
